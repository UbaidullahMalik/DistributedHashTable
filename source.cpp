...
public:
	...
	RingStruct* Parent_mac(int Mac_hash) {
		RingStruct* min_node = head;
		RingStruct* max_node = head;//max and min nodes of list
		while (max_node->next != head)
		{
			max_node = max_node->next;// Calculating maximum node
		}

		if (Mac_hash <= min_node->hashed_machine_id || Mac_hash > max_node->hashed_machine_id)
		{
		...
		else
		{
			...
			RingStruct* temp_node = head;
			while (Mac_hash > temp_node->hashed_machine_id)
			{
				temp_node = temp_node->next;
			}
			return temp_node;//returning machine (parent)
		}
	}
	...
	void set_routingtable() {
		...
		RingStruct* temp_node = head;
		RingStruct* parent;//parent of machine whichuser entered
		int sys_bits = log2(identifier_space);//getting bits of system

		do
		{
			int i = 1;
			while (i <= sys_bits)//Loop runs till system bits
			{
				int formula = pow(2, i - 1);
				//Calculating successor according to the formula p=2 raise to power i-1
				parent = Parent_mac(calculatehash((temp_node->hashed_machine_id) + formula, identifier_space));
				//Inserting machine into routing table(doubly list)
				temp_node->RT.insertnode(parent);
				i++;
			}
			temp_node = temp_node->next;//Moving to next mac
	...
	void clear_routingtable() {
		...
		do
		{
			temp_node->RT.clear();//Calling clear function in routing table list
			temp_node = temp_node->next;
		} while (temp_node != head);
	}
	...
	{
		RT_node* temp_node = obj->RT.geth();// Getting head node of routing table list
		while (temp_node)
		{
			//Getting hashed machine ids of each machine in routing table of a specific machine
			cout << temp_node->Obj->hashed_machine_id << " ";
			temp_node = temp_node->next;//moving ono nexr machine
		}
	}

	// Function for returning machines based on indexes
	RingStruct* ReturnMachine(RingStruct* Obj, int check)
	{
		int count = 0;
		RT_node* temp_node = Obj->RT.geth();//getting machine at head of list


		while (temp_node && count != check)
		{
			count += 1;
			temp_node = temp_node->next;// traversing towards the desired machine
		}
		if (!temp_node)
		{
			return NULL;
		...
		{
			return temp_node->Obj;// Returning the desired MAchine
		}
	}

	// Function for setting and checking routing table for each machine
	RingStruct* Mac_Routing(int hash_value, RingStruct* req_mac, bool loopedcase)
	{
		cout << " ----> " << req_mac->hashed_machine_id;//Printing arrow for saperating each mac

		//Returning machine if hashed id maches its own id
		if (req_mac->hashed_machine_id == hash_value)
		{
			cout << "\n";
			return req_mac;//returning machine
		}

		if (req_mac->hashed_machine_id > hash_value)
		{
			if (Parent_mac(hash_value) == req_mac)
			{
				cout << "\n";
			...
			else
			{
				...
				int j = 0;
				RingStruct* curr_mac = ReturnMachine(req_mac, j);
				RingStruct* next_mac = ReturnMachine(req_mac, j + 1);

				while (j < log2(identifier_space) - 1 && hash_value < next_mac->hashed_machine_id)
				{
					j++;
					curr_mac = next_mac;//moving onto the next mac for checking
					if (j + 1 < log2(identifier_space))
						next_mac = ReturnMachine(req_mac, j + 1);//returning J+1 th machine
				}
				//Looped case is a special case when key > max mac & key < min mac
				return Mac_Routing(hash_value, curr_mac, loopedcase);//Recursive function
			}
		}
		...
		if (req_mac->hashed_machine_id < hash_value)
		{
			RingStruct* curr_mac = ReturnMachine(req_mac, 0);
			// If jashed key is smaller
			if ((hash_value <= curr_mac->hashed_machine_id) || (loopedcase && curr_mac == head))
			{
			...
			else
			{
				...
				while (j < log2(identifier_space) - 1 && hash_value > next_mac->hashed_machine_id)
				{
					...
					curr_mac = next_mac;
					if (j + 1 < log2(identifier_space))
						next_mac = ReturnMachine(req_mac, j + 1);
					//Breaking loop if its a looped case
					if (loopedcase && next_mac == head)
						break;
				}
				// Recursive function
				return Mac_Routing(hash_value, curr_mac, loopedcase);
			}
		}
	...
	bool checkcase(int hash_value)
	{
		return (Parent_mac(hash_value) == head);
	}
